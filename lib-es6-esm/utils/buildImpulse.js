...
export function buildImpulse(options) {
    const rate = this._context.sampleRate, length = rate * options.seconds, decay = options.decay, impulse = this._context.createBuffer(2, length, rate), impulseL = impulse.getChannelData(0), impulseR = impulse.getChannelData(1);
    let n, i;
    for (i = 0; i < length; i++) {
